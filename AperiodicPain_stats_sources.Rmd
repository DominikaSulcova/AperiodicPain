...
```{r,message=FALSE,warning=FALSE}
# read .csv files
data_all <- read_csv(file.path(wd_path, paste0('AperiodicPFC_export_sources.csv')))
...
data_all <- data_all %>%
  ...
  filter(latency > 0) %>% 
  mutate(region = if_else(region == "Vis", "Visual", region))
data_all <- data_all %>%
  group_by(subject) %>%
  mutate(
...
for (ar in areas) {
  for (var in variables) {
      ...
      # display boxplot
      if(var == "amplitude") {
        output$boxplot_group <- output$boxplot_group %>%
...
for (var in variables) {
    ...
    # summarize and plot
    output <- summarize_and_plot(data_ratings, var, group_vars = 'area')
...
data <- data_all %>%
  filter(component == "N2") %>% 
  select(c('subject', 'age', 'component', 'dominant', 'area', 'region', 'rating', 'rating_norm', 'exponent', 'offset')) 
...
# try computing a separate model for the brain regions
data_PFC <- data %>% 
  filter(region == "PFC")
model_PFC <- lmer(rating ~ exponent + area + (1 | subject), data = data_PFC)
anova(model_PFC)
outliers <- check_model(model_PFC, data_PFC)

data_visual <- data %>% 
  filter(region == "visual")
model_visual <- lmer(rating ~ exponent + area + (1 | subject), data = data_visual)
anova(model_visual)
